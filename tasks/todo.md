...
- [ ] **Flip global default to gpt-4o-mini (step 4)** — single config change: `hermes config set model.default openai/gpt-4o-mini` (verify on srilu-vps first, then bulk-deploy across fleet). Expected savings: **~$3,550/yr at 10-customer mid-mix (proven by parity test, not estimated)**; quality: gpt-4o-mini matches kimi-k2-thinking at 93.3% on dispatcher matrix (see `tasks/dispatcher-parity-report.md`, 2026-05-05). **Pre-flip checklist:**
  ...
  - [ ] (c) catering's `handle_catering_owner_approval` LLM-drafted-quote prose A/B'd on 5–10 real inquiries (truth-guard intact: headcount + ISO date + menu items match inquiry?). **Awaits real catering inquiries; not in our control to manufacture.** Three options: (i) wait, (ii) seed synthetic inquiries through a one-time test, (iii) ship + rollback plan.
  - [ ] (d) EOD reconciliation gets explicit "show your math, then state the final number" prompting appended to the SKILL — **easy SKILL.md edit, ~10 min**.
...
- [ ] **EOD SKILL "show your math" prompting** (NEW 2026-05-05 — step 4 gate (d)) — append to `src/agents/shift/skills/handle_eod_reconcile/SKILL.md` (or equivalent — find the right SKILL): "Show your math step-by-step before stating the final number. List each receipt/sale individually with dollar amounts, then sum at the end." gpt-4o-mini drifts on multi-step arithmetic without this scaffolding (per benchmark profile in earlier discussion). ~10 min SKILL edit + verify with deploy + watch one EOD cron fire. Trivial; do this BEFORE flipping step 4.
