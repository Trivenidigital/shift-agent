...
- [x] F0029 exact-reference-edit quality gate (2026-05-18): customer attached an existing Lakshmis Kitchen flyer and asked to remove an extra `08:00` plus add an item for `$9.99`, but the app generated a new low-quality flyer titled `Uploaded Flyer Template`.
  ...
  - [x] Add source-preserving image-edit generation path for uploaded flyer corrections.
  - [x] Migrate source-preserving edits to OpenRouter-only provider posture (`OPENROUTER_API_KEY`, `flyer.edit_image_model`, default `openai/gpt-image-1`) so production does not require a separate direct OpenAI key.
...
- [x] Flyer edit-flow hardening follow-up (2026-05-18): rigorous testing found that source-preserving edits still had production gaps: missing source-edit provider credentials made source edits fail, unclear revisions could clear active previews/finals, natural edit language was under-parsed, and manual edit queue copy promised more than the system could complete automatically. Source-edit credentials now use the same OpenRouter key path as generation/vision.
  - Drift-check tag: extends-Hermes
