...
- [ ] Flyer Studio concierge intake for returning vague starts (2026-05-23): warm welcome back prompt should offer "one message" or "guide me step by step" instead of defaulting to sample ideas.
  ...
  - [x] Deploy to `main-vps` as `deploy-20260523-224614-8718f6ad`.
  - Deploy review: full tarball pytest gate passed with `1742 passed, 683 skipped, 22 warnings`; deploy smoke passed; pilot readiness remained `READY` (`16 passed, 0 failed`); gateway and cockpit active; WhatsApp bridge health `{"status":"connected","queueLength":0}`; deterministic Flyer final-package smoke returned `ok=true`; deployed `handle-flyer-intake` temp-state smoke returned `action="concierge_choice"` and persisted `status="concierge_awaiting_choice"`, `source="new_flyer"`.
  - [x] Fix and deploy live-path regression found 2026-05-24.
  - Regression review: live audit showed cf-router had new concierge code but `/opt/shift-agent/schemas.py` was stale, so `handle-flyer-intake` rejected `concierge_awaiting_choice` and the next `I am an existing customer` reply was swallowed as an active-project revision. Hotfixed the live schema, then shipped durable fixes: deploy now gates installed schema parity before gateway restart, existing-customer replies route to concierge before active projects, and concierge waiting state treats existing-customer replies as vague continuation. Verification: focused suite `285 passed`; full tarball gate `1745 passed, 683 skipped, 22 warnings`; deployed to `main-vps` as `deploy-20260524-003342-577753e8`; schema parity `ok`; gateway active; bridge connected queue `0`; installed cf-router probe returned `{'action': 'skip', 'reason': 'cf-router flyer concierge choice sent'}` for `I am an existing customer`.
  - [x] Fix customer-visible fact loss found in live Lakshmi's Kitchen grand-celebration flyer.
  - Fact-fidelity review: live F0089 locked only profile facts, so the generator was not forced to include the one-year celebration title, 05/30 and 05/31 dates, 30% dine-in sale, 20% take-away sale, Biryani BOGO, or Special Lunch Thali $12.99. Added exact live-request regressions for fact extraction, project creation, and renderer prompt copy. Initial red run failed on missing `campaign_title`, `event_date`, and offer facts. Two parallel reviewers then caught false campaign-title/date risks, direct "use saved business name" fact loss, run-on BOGO parsing, coupon false positives, date prompt/manifest divergence, `offer_price` duplication, and source-edit prompt copy drift; all were fixed with regression tests. Verification after review fixes: fact/render/generation/QA suite `160 passed`; concierge/router/static suite `285 passed, 119 skipped`; compileall and `git diff --check` passed.
