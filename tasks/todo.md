...
- ✅ 2026-04-28 — **Build `lookup-prior-leads-by-phone` script** — C02-Option-C foundation per `docs/catering-edge-cases.md` (v3.1) C02 case. Shipped via PR #26 (squash-merged). 22 tests; full Plan→5→Design→5→Build→PR→5 pipeline applied. v3.1 C02 case now `[RUNNABLE]`.
- [ ] **SKILL preamble integration for `lookup-prior-leads-by-phone`** — follow-up to PR #26. Catering SKILL Python preamble must invoke the deployed script subprocess-style and inject the returned dict into Kimi's prompt context. Without this, the script is unused-but-deployed infrastructure. Hermes-side change (modifies `parse_catering_inquiry/SKILL.md`'s preamble code). ~half-day. Includes: subprocess invocation pattern, JSON dict parsing with `lookup_status` field branching, prompt-template injection for the "returning customer, last booking N days ago" framing.
- [ ] **Hardening: `oserror:` status handling in `safe_load_json` consumers** — silent-failure-hunter NEW-1 from PR #26. The lookup script now distinguishes oserror from corrupt; `create-catering-lead:293` and `apply-catering-owner-decision:246` still treat oserror as fall-through-empty (silent I/O failure). Unified hardening PR adds explicit oserror handling at all 3 sites. ~1 hour.
- [ ] **Lock target migration: writer-side flock pattern** — silent-failure-hunter NEW-5 from PR #26. `lookup-prior-leads-by-phone` locks data fd directly; `create-catering-lead` uses `safe_io.FileLock` which locks `.lock` sibling. The two locks don't actually serialize against each other. Either (a) extend safe_io.FileLock to support data-fd locking, or (b) migrate the lookup to safe_io.FileLock. Currently OK because writer hold-times are well under 1s (lookup retry budget = 3s). Track for v0.3 platform cycle.
- [ ] **Add past-date validation to `create-catering-lead`** — v3.1 C10 locks the contract to "reject past dates with `ValidationError`" but the deployed script doesn't yet enforce it (only ISO format pattern in schema). Small ticket: parse `event_date` as a `datetime`, compare against `customer_now(cfg.customer.timezone).date()`, raise `ValidationError` if past. ~1 hour with tests. Unblocks v3.1 C10 from design-spec-pending to runnable.
