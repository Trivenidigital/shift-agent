...
1. **Number allowlisting** — add the customer number to the per-feature FLYER_* allowlists on /root/.hermes/.env (edit the symlink TARGET, never sed the symlink; incident protocol: backup, one-line assert, restart, /proc verify). **Call-out: FLYER_PREMIUM_OVERLAY_ALLOWLIST is SHARED by FOUR gates (overlay / det-recovery / det-first / CD-v2) — one env line in-scopes four features at once.** All allowlists are explicit-allow, empty=off.
