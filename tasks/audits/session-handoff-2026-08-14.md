...
- **~~P0 — brand-asset capture has no content classifier.~~ CLOSED** on branch `fix/flyer-brand-asset-classification`. The six substrings plus "does an active project exist" are gone: `_flyer_brand_asset_authorized` now asks `reference_extract.classify_reference_role` (which had zero cf-router call sites) for the routing verdict, vetoes the roles other paths own, and requires an explicit brand-artifact noun before mutating the store. An active project is context, never evidence; a classifier failure writes nothing. Gated by `tests/test_flyer_brand_asset_routing.py` in `flyer-premium-ci`. **Residual is smaller than first assumed** — measured against the live store, not inferred: of the 9 brand assets ever captured, exactly **one is active, B0008**, and its caption ("I want you to change theme of my fliers going forward…") is a genuine brand-template upload. B0009 and B0010 are already deactivated; B0001/B0002/B0004–B0007 are inactive. So no mis-captured asset is currently reaching a generation prompt and there is nothing for the operator to clean up. The fix is still prospective only.
