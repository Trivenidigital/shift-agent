...
- **P0 — brand-asset capture has no content classifier.** `hooks.py:4295-4296` is six substrings plus "does an active project exist". No pixels inspected; `reference_extract.classify_reference_role` exists and is unused here. An **uncaptioned** receipt from a customer with an active project still lands as `kind="logo"` and is fed to every later generation prompt. #694/#697 fixed routing precedence, not classification. **Unscheduled — highest-value remaining item.**
