...
|---|---|---|---|
| **B-HIGH-1** deposit-onboarding runbook | deferred to docs PR | ✅ **DONE** | `docs/runbooks/commerce-deposit-onboarding.md` — 247 lines, covers template config (Step 2), test fixtures/smoke (Step 4), kill switch `deposit_pct=0` (Steps 3+7), `url_status="unconfigured"` audit signal (Step 5), and references PR #321/#324/#327 |
...
| **Cockpit "Deposit-pending leads" tab** | carried from slice-1 | ⏸ **slice-3-gated** | `commerce-deposit-onboarding.md:230` lists it under "What slice 3 will add"; premature now (0 deposit rows, no `deposit_status` on any lead — see §2) |
