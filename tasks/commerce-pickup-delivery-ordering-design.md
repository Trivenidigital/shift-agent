...
**Live-deploy note (verified 2026-05-30):** production main-vps runs `deploy-20260530-034606-f1ff0cb9` (SHA `f1ff0cb`), **15 commits behind** origin/main `43fceab`. The only customer-facing undeployed runtime change is **#374** (`send-catering-ack` empty-JID fail-closed) — it rides the next normal deploy. The undeployed `safe_io.py` send-path tripwire is pytest-gated (inert in prod). Commerce is **dormant** (`enabled=False, provider=placeholder`). None of this design depends on deploying anything; it builds on origin/main.
