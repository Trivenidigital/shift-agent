...
| γ | Forbidden-completion-verbs lint | (merged + deleted) | **MERGED 2026-05-26** — PR [#263](https://github.com/Trivenidigital/shift-agent/pull/263), squash `8d61b56`. Measure/test mode only — no runtime hookup, not deployed by intent. Code-only on `origin/main` until PR-ζ wires it into the chokepoint. |
| δ | `mutation_class` field | `fix/flyer-action-registry-mutation-class` | **IN PROGRESS 2026-05-26** — adds required `mutation_class: Literal["local_reversible", "external_irreversible"]` field on `FlyerActionDefinition`. Annotates all 11 entries: 10 `local_reversible` (read-only + JSON-state-write) + 1 `external_irreversible` (`change_plan`, the only payment-triggering action). Field only; rollback handler wiring + audit-fail-closed behavior deferred to PR-ζ. |
