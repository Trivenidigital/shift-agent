...
`get_compliance_deadlines` and `get_equipment_maintenance_due` are owner-only and
...
from . import compliance_tool, equipment_tool, location_tool


def register(ctx) -> None:
    """Hermes plugin entry point, called once at gateway startup."""
    for tool in (compliance_tool, equipment_tool, location_tool):
        ctx.register_tool(
            name=tool.TOOL_NAME,
