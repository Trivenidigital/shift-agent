...
`get_compliance_deadlines`, `get_equipment_maintenance_due`,
`get_pending_catering_approvals` and `get_roster_capabilities` are owner-only and
enforce that in their handlers. `find_nearest_location` is deliberately public
and exposes only configured public store facts. Describing the plugin as
...
from . import (catering_approvals_tool, compliance_tool, equipment_tool,
               location_tool, roster_tool)


def register(ctx) -> None:
    """Hermes plugin entry point, called once at gateway startup."""
    for tool in (catering_approvals_tool, compliance_tool, equipment_tool,
                 location_tool, roster_tool):
        ctx.register_tool(
            name=tool.TOOL_NAME,
