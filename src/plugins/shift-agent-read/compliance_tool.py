...
THE STATE DISTINCTION IS THE POINT. Three outcomes that must never collapse:
...
without inference. Refusals return no `items` key at all, so no failure path
produces a shape that reads as an authoritative "nothing due".
"""
...
from identity import ok, require_owner  # noqa: E402  (sibling module, flat layout)
...
# function.description, which removes the tool's capability line from the
# deferred catalog and makes it undiscoverable.
SCHEMA = {
    "name": TOOL_NAME,
...
def _today() -> date:
    """Today in the customer's timezone; UTC-ish fallback if config is unusable."""
...
def handler(args=None, **kwargs) -> str:
    ...
    try:
        ...
        return ok(source_status="unreadable", window_days=window_days,
                  tracked_total=0, in_window=0, items=[])
