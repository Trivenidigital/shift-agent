...
stub — one SKILL.md, no store, and (verified by grep against
dispatch_shift_agent/SKILL.md) no dispatcher routing row. It was deployed via
tools/skills-manifest.txt and structurally unreachable: nothing could invoke it,
and had something invoked it there was no data to answer from.

This script is the write half of the agent's first useful workflow. The read
half is inline jq in the SKILL — per the Hermes-first rule, "sort a JSON list by
date and describe it" is model work, not Python.

Operator/owner tooling: the SKILL never calls this. Agent #19's first-workflow
