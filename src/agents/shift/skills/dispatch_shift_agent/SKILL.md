...
# Look up across the five pools, in this priority:
...
jq --arg c "$CODE" '.followups[] | select(.approval_code == $c) | select(.status == "awaiting_owner_approval" or .status == "approved_sent")' /opt/shift-agent/state/catering-followups.json   # M5 catering follow-up → approve-catering-followup
```
