"""Tests for audit_helpers.scan_orphan_pushes — platform-helpers consolidation
commit 3 of 4.
...
from audit_helpers import scan_orphan_pushes


def _lead(*, expense_id: str, status: str = "APPROVED_PENDING_PUSH",
          approved_at: datetime | None = None,
          reconcile_required: bool = False) -> SimpleNamespace:
...
def test_no_pending_leads_returns_empty(tmp_path):
    flagged, ids = scan_orphan_pushes(
        [_lead(expense_id="E1", status="PUSHED")],
        pending_status="APPROVED_PENDING_PUSH",
        completion_types=("expense_pushed",),
        id_attr="expense_id",
        timestamp_attr="owner_approval_received_at",
...
def test_stale_pending_no_audit_match_is_orphan(tmp_path):
    ...
    flagged, ids = scan_orphan_pushes(
        leads,
        pending_status="APPROVED_PENDING_PUSH",
        completion_types=("expense_pushed", "expense_push_failed"),
        id_attr="expense_id",
        timestamp_attr="owner_approval_received_at",
        log_path=log,
    )
...
def test_custom_stale_seconds_kwarg(tmp_path):
    """Caller can override the 60s default."""
    log = tmp_path / "decisions.log"
    log.write_text("")
    ...
    _, ids_default = scan_orphan_pushes(
        leads,
        pending_status="APPROVED_PENDING_PUSH",
        completion_types=("expense_pushed",),
        id_attr="expense_id",
    ...
    _, ids_short = scan_orphan_pushes(
        leads2,
        pending_status="APPROVED_PENDING_PUSH",
        completion_types=("expense_pushed",),
        id_attr="expense_id",
...
def test_other_status_audit_entries_are_ignored(tmp_path):
    """Audit entries whose type isn't in completion_types must NOT exclude orphans."""
    log = tmp_path / "decisions.log"
