...
class TestF7PrimaryMode:
    ...
    def test_proposal_request_generates_deterministically_when_flag_enabled(self, mods, state_env):
        """PR-B2 (2026-07-21): a PLAIN proposal request against an active lead is
        GENERATED by cf-router via create-catering-proposal-options
        --auto-generate-from-menu (skip + f7_proposal_request_deterministic_generation).
        Reverses PR-A, which had escaped active-lead generation to the Hermes creative
        path (no live evidence it worked — the real gpt-4o-mini gateway spiraled)."""
        hooks_mod, actions_mod = mods
        ...
        # Generate deterministically → cf-router skips the LLM.
        assert result == {
            "action": "skip",
            "reason": "cf-router F7 primary: proposals generated deterministically for L0001",
        }
        mock_create.assert_called_once()  # cf-router generates against the active lead
        assert mock_create.call_args[0][0] == "L0001"
        mock_reply.assert_not_called()
        rows = [json.loads(l) for l in state_env["log_path"].read_text(encoding="utf-8").splitlines() if l.strip()]
        audits = [r for r in rows if r.get("type") == "cf_router_intercepted"]
        assert len(audits) == 1
        assert audits[0]["reason"] == "f7_proposal_request_deterministic_generation"

    def test_proposal_request_deterministic_generation_bypasses_llm(self, mods, state_env):
        """PR-B2 companion: a differently-phrased plain proposal ask against an active
        lead also generates deterministically inside cf-router (the LLM is never
        reached for generation). Mix-and-match recompose is the only proposal work that
        still escapes to Hermes — see test_catering_pra_reachability."""
        hooks_mod, actions_mod = mods
        hooks_mod.F7_PROPOSAL_BRANCH_ENABLED = True
        _seed_config(state_env)
        _seed_leads_multi(state_env, [
