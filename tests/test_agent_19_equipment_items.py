"""Agent #19 Wave-1 — the seed path for state/equipment-items.json.

Agent #19 shipped as a self-declining stub with no store: it was in
tools/skills-manifest.txt (so it deployed) and had no data to answer from. These
tests pin the write half of the fix — the store can be seeded, and seeding is
auditable. The read half is `get_equipment_maintenance_due`, covered by
tests/test_shift_agent_read_equipment_tool.py.

Store tests are Linux-only (fcntl via safe_io) and subprocess-invoked, mirroring
tests/test_agent_13_compliance_script.py.
"""
...
def _env(env_dir):
    return {
        **os.environ,
        ...
        "SHIFT_AGENT_EQUIPMENT_ITEMS_PATH": str(env_dir / "state" / STORE_NAME),
...
def test_multiple_assets_sort_by_service_date_for_the_read_path(env_dir):
    """The read tool sorts by days_until; this pins that the data supports it."""
