...
def test_failed_send_does_not_arm_dedup_window(tmp_path):
    """§12b contract: a FAILED delivery must NOT arm the dedup window — otherwise
    a transient failure would suppress both the retry and its dead-letter trail.
    Only DELIVERED messages arm the window."""
    state = tmp_path / "dedup.json"
    fail = _make_fake_notify_bin(tmp_path, exit_code=2)
    r1 = notify_owner_with_fallback("t", "m", source="s", notify_owner_bin=str(fail),
                                    notify_failed_log=tmp_path / "log",
                                    dedup_state_path=state, dedup_window_min=30)
    assert r1 is False
    assert (tmp_path / "log").exists()  # dead-letter written on the failed send
    # Same message, now the notifier works -> MUST attempt (the earlier failure
    # did not arm the window).
    ok_bin, count = _counting_bin(tmp_path)
    r2 = notify_owner_with_fallback("t", "m", source="s", notify_owner_bin=str(ok_bin),
                                    notify_failed_log=tmp_path / "log2",
                                    dedup_state_path=state, dedup_window_min=30)
    assert r2 is True
    assert _invocations(count) == 1
