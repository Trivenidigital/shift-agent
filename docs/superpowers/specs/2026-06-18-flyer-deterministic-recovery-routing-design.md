...
### 8.1 Implementation notes (mechanism that emerged during Codex review)
- **Single context-var gate, not per-helper threading.** Forcing mode 2 is implemented via `_FORCE_BACKGROUND_ONLY` (a `ContextVar`, set inside `_render_model`/`build_image_generation_prompt` when `force_background_only=True`): `_integrated_poster_eligible` returns `False` while it is set, which flips `_background_only_eligible` to `True` at **every** prompt site at once — avoiding the leak-prone whack-a-mole of threading a flag through each prompt helper. The one unconditional item directive (`_poster_copy_block`) emits names-only-for-imagery under force; genuine background-only is unchanged.
- **Persisted `FlyerProject.deterministic_recovery` flag.** The transient context-var only covers the draft re-render; the **final-export** is a separate invocation that re-reads `projects.json`. Without persistence it would treat a recovered (integrated-eligible) project as a `direct_poster_source` and crop the 4:5 overlay to the 1:1 / 9:16 finals (dropping required text). The rung therefore persists `deterministic_recovery=True` on success; `_integrated_poster_eligible` also returns `False` for such a project, so the final export re-applies the overlay per output size. **Accepted by-design (operator 2026-06-18):** this additive field serializes a default-`false` key into project state JSON on rewrite — the same pattern as the existing `premium_repair_qa` field — which is behavior-preserving (flag off changes no code path). It is **not** a Codex blocker; it is the minimal correct persistence for cross-invocation consistency. The lower-risk alternative (deriving recovery from the raw-background sidecar inside `render_final_package`) was rejected to avoid regressing the delicate source-edit / operator-upload / stale-raw provenance logic.
